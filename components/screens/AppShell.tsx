...
export function AppShell({ active, children, title, headerAction }: Props) {
  return (
    ...
    >
      {/* status bar */}
      <div className="flex items-center justify-between px-6 pt-1 pb-1.5 text-[11px] font-semibold text-[#0F172A]/80 tracking-tight">
        <span>9:41</span>
      ...
      {(title || headerAction) && (
        <div className="flex items-end justify-between px-5 pt-2 pb-1">
          <h1 className="text-[22px] font-bold tracking-tight text-[#0F172A] leading-none">
            {title}
          </h1>
      ...
      <div className="shrink-0 px-3 pt-2 pb-2">
        <div
          className="rounded-[24px] border border-[#0F172A]/5 bg-white/90 backdrop-blur-xl px-1.5 py-1 flex items-center justify-around"
          style={{
            boxShadow:
              "0 14px 30px -12px rgba(15,23,42,0.2), 0 1px 0 rgba(255,255,255,0.8) inset",
          }}
        >
          <TabItem
            label="Calendar"
      ...
      <div className="shrink-0 flex justify-center pb-1">
        <div className="w-[120px] h-[4px] rounded-full bg-[#0F172A]/25" />
      </div>
    </div>
  );
}
...
function TabItem({
  ...
  return (
    <div className="flex flex-col items-center gap-0 px-1 pt-1 pb-0.5 w-[46px]">
      <div className={active ? "text-accent" : "text-[#94A3B8]"}>{icon}</div>
      <span
    ...
    <div className="flex flex-col items-center gap-0 -mt-4">
      <div
        ...
        style={{
          ...
          boxShadow: "0 10px 20px -6px rgba(99,102,241,0.55)",
        }}
      >
...
function SignalDots() {
  return (
    <div className="flex items-end gap-0.5">
      <span className="w-0.5 h-1.5 bg-[#0F172A]/80 rounded-sm" />
      <span className="w-0.5 h-2 bg-[#0F172A]/80 rounded-sm" />
      <span className="w-0.5 h-2.5 bg-[#0F172A]/80 rounded-sm" />
      <span className="w-0.5 h-3 bg-[#0F172A]/80 rounded-sm" />
    </div>
  );
}
...
function BatteryIcon() {
  return (
    <div className="relative flex items-center">
      <div className="w-5 h-2.5 rounded-[3px] border border-[#0F172A]/80 flex items-center p-[1px]">
        <div className="h-full w-3/4 bg-[#0F172A]/80 rounded-sm" />
      </div>
      <div className="w-0.5 h-1 bg-[#0F172A]/50 rounded-r-sm ml-[1px]" />
    </div>
  );
}
